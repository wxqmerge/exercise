...
A Day pulldown (bottom row of the workout screen and the summary screen) overrides the auto day for the session (`selectedDay` in `App.jsx`; not persisted). Workout entries are stored per type and day in localStorage (`exercise-entries`: `{ [workoutType]: { [day]: { [exerciseId]: { reps, weights } } } }`), saved automatically on every edit (reps/weight typing, nudge buttons) via `persistForm` — so values survive navigating to Settings or going Back without pressing Next — and cleared by "Start over". Entries are also synced with the server: the app pushes the full map to `PUT /api/entries` after every save and clear ("Start over"), and on load pulls `GET /api/entries` and merges it under local values (local wins; server values fill gaps) — so entries survive a browser wipe and work from any browser/device. The server copy lives in `data/entries.json` (gitignored); a PUT replaces the stored map, so a cleared day is also removed from the server. Legacy stores keyed by day only are auto-migrated under the `dumbbells` type on read. The reps/weight form shows the current exercise's saved values on mount, after a day/type change, and after Next/Back navigation (so Next without edits is a no-op for any exercise; missing values fall back to defaults). Weights are restricted to 5 lb increments: they are snapped to the nearest multiple of 5 on blur, on every autosave, and when saved values load (nudge buttons step by ±5). Typing a weight fills the other empty sets only once the value has at least two digits (so partial input doesn't propagate). The config error screen offers a Retry button for non-401 failures.
...
- The static builder (`scripts/build-static.py`) reads `PROGRAMS` and emits `exercise/index.html` (hub) + one page per routine (`exercise/dumbbells.html`, `exercise/hotel.html`). Each exercise title carries a `.log` span (data-type/data-day/data-ex) that a small inline script fills at load by fetching `GET /api/entries` from the server — the API base (`DOMAIN`/`PORT`) and `APP_KEY` are read from `server/.env` and embedded at build time, so the cards work even when opened from `file://` on a phone. It renders one value each (the last non-empty set) as `10-reps 35-lbs` (missing parts omitted; the span stays hidden when the exercise has no entry). If the server is unreachable, it falls back to the browser's `exercise-entries` localStorage. `hotel.md`/`routine.md` are human-readable copies, not sources of truth.
...
- **`/api/entries`**: GET returns the stored entries `{ [type]: { [day]: { [exerciseId]: { reps, weights } } } }` (from `data/entries.json`; `{}` when none). PUT accepts the same nested shape (full replace; values validated and coerced to strings, at most 3 per set array) and persists to `data/entries.json`.
